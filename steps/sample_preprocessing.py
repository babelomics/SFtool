    ...
    for sample in ctx.samples:
        ...
        for category in categories:
            if category == 'PR' or category == 'RR':
                ...
                sample.vcf_outputs["PGx_preprocessed"] = pharmCAT_vcf_preprocessor(str(sample.vcf.resolve()), ctx.config.paths.python, ctx.config.paths.pharmCAT, ctx.config.paths.bcftools, ctx.config.paths.htslib)
